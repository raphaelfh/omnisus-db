...
**Data:** 09/09/2026  
**Referência original de código:** commit `e681aceb869004dc192c4aaa588a51adc6d54ac7`  
**Revisão adversarial:** commit `011e77bdf14afcc3d72ebb666247d0968f92c165`; `src` e `tests` sem diferenças em relação à referência original.  
...
**Retificação:** a versão inicial misturou leituras de configuração feitas em momentos diferentes e atribuiu incorretamente a incompatibilidade de Python ao commit citado. A07 foi retirado; A02 passou a decisão de contrato e A05 foi reclassificado como P2. A [revisão adversarial](/Users/raphael/PycharmProjects/omnisus-db/reports/2026-09-09-revisao-adversarial.md) registra as contestações e a [versão original preservada](/Users/raphael/PycharmProjects/omnisus-db/reports/evidence/2026-09-09/adversarial/original-report.md) permite auditar as alterações. Os resultados da suíte abaixo são históricos, não uma nova execução de CI.
...
**Há bloqueios concretos para automatizar o runner FTP, usar a série IBGE documentada e atualizar registros CNES sem controles adicionais.** Há defeitos reproduzidos que permitem anunciar sucesso após falha, perder registros em atualização e alterar valores ao conciliar schemas. O append duplica registros quando um mesmo escopo é reenviado, o que exige política de reprocessamento na biblioteca ou no consumidor. A integração populacional consulta uma tabela inadequada para os anos documentados. Esses achados não invalidam automaticamente todo uso exploratório ou uma carga individual já reconciliada; não foi avaliada uma implantação específica.
...
Após a revisão, permanecem **10 grupos de defeitos: 4 P1 e 6 P2**, além de **uma decisão de contrato (A02)**. **A07 foi retirado por falta de sustentação no estado versionado.** Os IDs foram preservados para rastreabilidade. A09 agrega mais de uma falha de manutenção; a contagem não representa 10 causas-raiz independentes. Prioridade é julgamento técnico para os cenários descritos, não frequência medida de incidentes. Os riscos de capacidade e governança são apresentados separadamente.

## 2. Método, abrangência e limites

O inventário identificou 35 arquivos Python em `src`, totalizando 3.249 linhas físicas, e 56 arquivos Python em `tests`, incluindo inicializações. A inspeção concentrou-se nos fluxos de ingestão, lake, transformações, CLI, configurações e testes relevantes; esses totais não constituem prova de revisão exaustiva de cada linha. Há 11 datasets no registro FTP e importadores específicos para população IBGE e nomes de estabelecimentos CNES.

A auditoria original não congelou o working tree nem registrou hashes por arquivo durante a coleta. Por isso, a configuração de release foi lida em um estado misto e o anexo `verification.json` contém um commit declarado incompatível com os metadados de Python registrados. Esse anexo permanece preservado como evidência histórica, com essa ressalva. Na revisão adversarial, foram comparados os blobs Git, verificados os hashes de código/configuração antes e depois dos controles e confirmado o caminho do módulo Python importado. Não houve mudança desses arquivos durante os controles.
...
P1 indica correção prioritária por impacto em integridade ou completude nos fluxos descritos. P2 indica defeito relevante de operação, rastreabilidade ou compatibilidade, incluindo cenários sintéticos cuja incidência em dados reais não foi medida. “Contrato” indica uma decisão de comportamento, sem caracterizar violação de uma garantia vigente. A severidade não representa uma classificação CVSS.
...
| A02 | Contrato | Reprocessamento em append duplica registros; política a definir | Duas cargas da mesma fixture; comportamento de append confirmado |
...
| A05 | P2 | Evolução de schema não concilia tipos e pode alterar valores | Frames sintéticos; incidência em layouts reais não estabelecida |
| A06 | P1 | Upsert CNES pode apagar dados antes de falhar | Falha injetada após DELETE |
| A07 | Retirado | Configuração foi atribuída ao estado Git errado | No commit original, tudo 3.13; no atual, tudo 3.12 |
| A08 | P2 | Snapshot informado dentro da transação externa precede seu commit | Controle direto funciona; controle em transação falha |
...
**Decisão proposta:** definir quem garante reprocessamento seguro: biblioteca ou aplicação consumidora. São alternativas legítimas rejeitar escopos já carregados, pular versões idênticas ou substituir snapshots completos. Append explícito continua válido. Um manifesto interno é uma opção, não condição universal; um controle externo durável também pode cumprir o contrato, desde que lide com confirmação e retomada corretamente.

Se for escolhida substituição, validar identidade e completude da fonte antes de qualquer DELETE, impedir que uma resposta vazia inesperada elimine dados existentes, definir política para vazios legítimos e controlar disputa entre writers. A substituição deve usar a identidade lógica completa do escopo: CNES-ST particiona por ano/mês, mas isso não autoriza apagar as demais UFs da mesma partição física.

**Aceite condicionado à política escolhida:** repetir o escopo não altera os dados ou é rejeitado explicitamente antes de gravar. Se houver substituição, uma revisão válida troca a versão anterior; vazio inesperado e fonte incompleta preservam o estado anterior; outras UFs não são afetadas; disputa entre writers é detectada ou resolvida. Interrupção e retomada convergem sem duplicação. Não assumir que uma chave de registro é globalmente única sem validar o dataset.
...
**Reavaliação de prioridade:** P2 até demonstrar uma combinação de layouts reais afetados ou um contrato de entrada que exija promoção automática. O exemplo UInt8/UInt16 testa `Lake.ingest`; não representa a inferência usual de inteiros do parser DBF. O controle adicional com Int64/Float64 confirmou dependência da ordem: inteiro primeiro armazena 2; float primeiro preserva 1,75.
...
**Impacto:** falhas de escrita ao atualizar códigos existentes deixam nomes ausentes ou atualização parcial. O caso de perda de registro anterior aplica-se a refresh com `only_missing=False` ou códigos explícitos já presentes; o caminho padrão `codes=None, only_missing=True` seleciona somente ausentes. O processamento registro a registro também merece avaliação de custo de commits em cargas maiores. Um controle com a mesma falha injetada preservou a linha quando DELETE e INSERT foram envolvidos em transação, reforçando a causa identificada.
...
### A07 — Retirado: incompatibilidade de Python não sustentada

No commit `e681ace`, o manifesto e o lockfile declaram `>=3.13`, e o gate cria Python 3.13. No commit `011e77b`, manifesto e lockfile declaram `>=3.12`, e o gate cria Python 3.12. O job de build/publicação usar 3.13 no estado atual não contradiz o suporte mínimo a 3.12.

O relatório combinou leituras de momentos distintos. Seu próprio `verification.json` já registrava `requires_python >=3.12` e `gate_version_meets_package_requirement=true`. Não há evidência suficiente para atribuir o bloqueio alegado a qualquer um dos dois commits. **A07 é um erro do relatório, não apenas um defeito que se pode declarar corrigido posteriormente.**

Não permanece correção de produto associada a A07. Instalação limpa e disponibilidade de wheels continuam fora da validação desta auditoria e não devem ser inferidas como aprovadas ou reprovadas pela retirada do achado. As comparações versionadas estão em [controls.json](/Users/raphael/PycharmProjects/omnisus-db/reports/evidence/2026-09-09/adversarial/controls.json).

### A08 — ImportResult informa o snapshot anterior dentro de transação externa
...
O controle adversarial mostrou que `Lake.ingest` chamado diretamente, fora dessa transação, reporta o snapshot correto. Portanto, o defeito não se aplica indistintamente a toda chamada de ingestão; ele atinge o batching usado pelo runner e outros consumidores que abrem a transação externa.
...
No parser IBGE, somente o primeiro conjunto de resultados é considerado, valores não conversíveis são descartados e o parâmetro `year` não restringe os anos lidos. O experimento controlado pediu 2022 e forneceu artificialmente uma linha de 2023 e múltiplos resultados. Ele mostra falta de validação defensiva, mas não demonstra que o endpoint atual devolva esse formato: os metadados arquivados não apresentam classificações para esses agregados. Não se deve contar esse caso sintético como uma perda adicional confirmada de dados da fonte. A resposta vazia real de A03 permanece evidência independente.
...
- [Metadados originais da verificação](/Users/raphael/PycharmProjects/omnisus-db/reports/evidence/2026-09-09/verification.json), preservados com a inconsistência de proveniência de release descrita na seção 2, e [saída Frictionless](/Users/raphael/PycharmProjects/omnisus-db/reports/evidence/2026-09-09/frictionless.txt).
- [Reexecução dos dez grupos](/Users/raphael/PycharmProjects/omnisus-db/reports/evidence/2026-09-09/adversarial/repeated-observations.json), [controles adicionais e hashes](/Users/raphael/PycharmProjects/omnisus-db/reports/evidence/2026-09-09/adversarial/controls.json) e [script dos controles](/Users/raphael/PycharmProjects/omnisus-db/reports/evidence/2026-09-09/adversarial/controls.py).
...
|---|---|---|---|
| 1 | Restaurar confiabilidade dos fluxos afetados | A01, A03, A04 e A06; testes de falha, fonte real e rollback | Nenhuma falha omitida ou perda nos cenários descritos |
| 2 | Definir reprocessamento seguro | Decisão A02 e defeito A08; responsabilidades, política por escopo e snapshot confirmado | Repetição controlada e retomada sem duplicação conforme contrato aprovado |
| 3 | Corrigir manutenção e compatibilidade | A09, A10 e A12; operações reais de manutenção e quoting/URI | Comandos observáveis e funcionais; A07 não gera trabalho de correção |
| 4 | Consolidar semântica analítica | A05 e A11; tipos, dicionários, completude e proveniência | Valores e views consistentes com o contrato; matriz real de layouts verificada |
...
Para os fluxos recorrentes avaliados, as condições propostas são: escopos contabilizados, repetição controlada, rollback recuperável, fidelidade dos tipos, fonte/ano validados e snapshot correto. Proveniência durável pode ser atendida pela biblioteca ou pelo consumidor; um manifesto interno não é requisito universal. Manutenção e instalação precisam de validação própria quando fizerem parte da operação. Esses critérios são propostas para o desenho futuro, não requisitos previamente acordados com o usuário nem prova de falha de toda implantação possível.
